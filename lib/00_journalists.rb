...
puts "
                ********************************************************
                |         BIENVENUE à toi Jeune Entrepeneur            |
                |                                                      |
                |         Choisie un chiffre qui repondra              |
                |            à la question de ton choix                |
                |                                                      |
...
while choice != 0
    ...
    case choice.to_i
        when 1
            puts "Le nombre de handle de l'array FizzBuzz est: " + journa_list.length.to_s
        when 2 
            puts "le handle le plus court l'array FizzBuzz est: " + journa_list.min{|x,y| x.length <=> y.length}
        when 3 
            puts " Il ya << #{journa_list.count{|x| x.delete("@").length == 5}.to_s} >> handle contenant 5 carateres dans l'array FizzBuzz "
        when 4
            counter = 0 
            journa_list.each {|handle| counter += 1 if handle[1].match(/[A-Z]/) }
        ...
        when 7
            puts "Le handle << @epenser >> se trouve à la #{journa_list.index("@epenser").to_s} éme position"
        when 8 
            handle_length(journa_list)
        when 0
            break
        else
            puts ""
            puts "Hahaha pas de troll choisie un numéro entre 0 et 8"
            print "Choisie un numero:> "
            choice = gets.chomp
    end
    if choice != 0
        if choice != 3 || choice != 6 || choice != 8
            puts ""
            puts "Choisie une nouvelle action ou Zéro pour finir le programme"
            print "Choisie un numero:> "
            choice = gets.chomp     
        else
            print "Appuie sur une TOUCHE pour revenir Au menu"
            gets.chomp
            system("clear")
